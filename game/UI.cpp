...
void UI::Update()
{
    if (VignetteTransparencyUniformLoc == -1)
        VignetteTransparencyUniformLoc = GetShaderLocation(game->MainResources.Shaders["vignette"], "Transparency");
    ...
    string FINAL_STR = "DASH " + to_string((int)max((int) (((int)((1.0f - (GetTimeUtils() - game->MainPlayer.LastDashed)) * 10000.0f))/10.0f), 0)) + "ms";
    ...
    BeginShaderMode(game->MainResources.Shaders["vignette"]);

    float VignetteTransparency = game->MainPlayer.DisplayHealth / 100.0f;
    VignetteTransparency = 1.0f - VignetteTransparency;
    SetShaderValue(game->MainResources.Shaders["vignette"], VignetteTransparencyUniformLoc, &VignetteTransparency, SHADER_UNIFORM_FLOAT);
